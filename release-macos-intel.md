...
CI hanya menaruh `darwin-aarch64` + `windows-x86_64` di `latest.json`. Tambahkan
`darwin-x86_64`:
```bash
VER="1.11.0"
TAG="app-v${VER}"
...
# ambil latest.json yang sekarang
curl -fsSL -o latest.json "${BASE}/latest.json"

# signature = ISI file .sig; url = nama aset .app.tar.gz di GitHub (spasi -> titik)
SIG="$(cat dist-macos/*_x64.app.tar.gz.sig)"
URL="${BASE}/Auto.Clipper_${VER}_x64.app.tar.gz"

# sisipkan platform darwin-x86_64
...
# re-upload latest.json (timpa yang lama)
gh release upload "${TAG}" latest.json --clobber
```

Verifikasi isi `latest.json` sekarang punya 3 platform:
```bash
